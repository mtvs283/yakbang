  ...
  switch (type) {
    case "shipped":
      return {
        ...
        html: `<p>환자 ${ctx.name}이시여,</p><p>주문하신 <b>${ctx.remedy ?? "약"}</b>을 약방에서 보냈소이다. 배달이 시작되었으니 곧 거처에 닿을 것이오.</p><p>— 집현전 약방</p>`
      };
    case "arrived":
      return {
        subject: "약이 거처에 도착하였소이다",
        html: `<p>환자 ${ctx.name}이시여,</p><p>약이 거처에 무사히 도착하였소이다. 처방대로 드시고 한국어에 밝아지시오.</p><p>— 집현전 약방</p>`
      };
    case "reminder":
      return {
        subject: `환자 ${ctx.name}, 잘 지내시오?`,
        html: `<p>환자 ${ctx.name}이시여, 그간 잘 지내셨소? 약방에서 새 약이 나왔소이다. 한번 들르시오.</p><p>— 집현전 약방</p>`
      };
  }
}
